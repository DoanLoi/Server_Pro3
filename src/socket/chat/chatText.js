    ...
    socket.on("chat-text", (data) => {
      ...
      console.log(data)
        ...
        if (clients[data.contactid])//Neu nguoi muon ket ban dang onl
        {
          ...
          emitNotifyToArray(clients,data.contactid, io, "response-chat-text",response);
        }
      });

      socket.on("chat-text-admin", (data) => {
        let response=data;
       if (clients[data.userId])//Neu nguoi muon ket ban dang onl
       {
         //gui du lieu den cac socket cua nguoi ban muon ket ban
         emitNotifyToArray(clients,data.userId, io, "response-chat-text-admin",response);
       }
     });
